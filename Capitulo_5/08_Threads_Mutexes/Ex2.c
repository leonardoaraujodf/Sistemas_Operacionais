...
void* char_print (void* parameters) //QUAL CARACTER OU QUALQUER COISA
{
	struct char_print_parms* p = (struct char_print_parms*) parameters; // INTERPRETA COMO A ESTRUTURA QUE FOI CRIADA ACIMA, O PRIMEIRO BYTE UM CARACTER E OUTROS UM INTEIRO, PONTEIR PARA ESTRUTURA
	int i;
	for (i = 0; i < p->count; ++i)
...
int main ()
{
	...
	thread1_args.character = 'x';//PRIMEIRA DAS ESTRUTURAS VAI RODAR 100 VEZES
	thread1_args.count = 100;
	pthread_create(&thread1_id, NULL, &char_print, &thread1_args);
	thread2_args.character = 'o';
	thread2_args.count = 80; //COMO NÃO USEI JOIN AINDA DESSAS TRHEADS VAI MORRER E VOU PERDER OS DADOS DO OUTRO
